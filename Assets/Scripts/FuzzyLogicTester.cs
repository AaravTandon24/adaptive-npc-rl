...
public class FuzzyLogicTester : MonoBehaviour
{
    ...
    public static void RunTests()
    {
        ...
        // Case 1: Tier UP — high win rate, full HP (fast clean kills)
        DifficultyTier tier = classifier.Evaluate(0.9f, 50f, 1.0f, 1);
        AssertEqual(DifficultyTier.Hard, tier, "0.9 WR & full HP should Tier UP to Hard");

        // Case 2: Tier UP again to Expert
        tier = classifier.Evaluate(0.9f, 50f, 1.0f, 2);
        AssertEqual(DifficultyTier.Expert, tier, "0.9 WR & full HP should Tier UP to Expert");
        ...
        tier = classifier.Evaluate(0.1f, 5f, 0.0f, 7);
        AssertEqual(DifficultyTier.Easy, tier, "Should clamp at Easy");

        // Case 6: Reset to Medium
        tier = classifier.Evaluate(0.9f, 50f, 1.0f, 8);
        AssertEqual(DifficultyTier.Medium, tier, "Reset to Medium");

        // Case 7: Near threshold — win rate borderline, low HP (took damage)
        // winRate_High(0.7) = 0.0, PlayerHP_High(0.5) = 0.25
        // min(0.0, 0.25) = 0.0 <= 0.6 => should NOT tier UP
        tier = classifier.Evaluate(0.7f, 30f, 0.5f, 9);
