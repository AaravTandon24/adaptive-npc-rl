...
public class FuzzyTierClassifierTests
{
    ...
    public void TestFuzzyLogicTransitions()
    {
        ...
        // avgSurvivalTime = 50f (survivalTime_High = 1.0f)
        // min(1.0, 1.0) = 1.0 > 0.6 => should tier UP
        DifficultyTier tier = classifier.Evaluate(0.9f, 50f, 1);
        Assert.AreEqual(DifficultyTier.Hard, tier, "High winRate & survivalTime should trigger Tier UP to Hard");

        // --- CASE 2: Tier UP again to Expert ---
        tier = classifier.Evaluate(0.9f, 50f, 2);
        Assert.AreEqual(DifficultyTier.Expert, tier, "High winRate & survivalTime again should trigger Tier UP to Expert");
        ...
        // Survival time is HIGH (45 s) to verify the old AND-rule with stLow is gone.
        // Tier DOWN must fire on win rate alone regardless of how long the player survives.
        ...
        tier = classifier.Evaluate(0.1f, 45f, 7);
        ...
        classifier.Evaluate(0.9f, 50f, 8);
        tier = classifier.Evaluate(0.9f, 50f, 9);
        ...
        // avgSurvivalTime = 30f — survivalTime_High = 0.333f
        // min(0.25, 0.333) = 0.25 <= 0.6 => should NOT tier UP
        tier = classifier.Evaluate(0.75f, 30f, 10);
